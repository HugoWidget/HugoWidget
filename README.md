...
- **简化维护流程**：提供自动化 PE 维护工具集，一键解除冰点还原、禁用服务、清理系统。
...
| **冰点还原管理**  | HugoFreezeApi, HugoFreezeDriver                         | 调用 API 或驱动级接口，查询/设置磁盘冻结状态                 |
| **希沃服务管控**  | HugoDisable, HugoLaunchTool, HugoInstaller, HugoProtect | 禁用服务、启停进程、安装/卸载希沃管家、开关文件保护          |
| **虚拟磁盘挂载**  | HugoMount                                               | 挂载/卸载希沃的配置盘和日志盘                                |
| **密码恢复**      | HugoPassword                                            | 破解希沃管理密码/锁屏密码（支持 V1/V2/V3）                   |
| **DLL 注入**      | HugoInjector                                            | 将指定 DLL 注入目标进程，结合 HugoHSSA.dll 实现高级解锁      |
...
- **编程语言**：C++ (主要)、Python、JavaScript (前端注入脚本)
- **构建工具**：Visual Studio 2022, PyInstaller
- **图形界面**：Qt6 + Qlementine 风格

## 📄 许可证

大部分项目采用 **GNU General Public License v3.0 (GPLv3)**。部分组件（如 HugoUtils）使用 **LGPLv3**，其他第三方依赖遵循其原始许可。详情请参阅各仓库中的 `LICENSE` 文件。
